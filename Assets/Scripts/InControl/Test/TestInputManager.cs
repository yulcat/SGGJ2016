...
namespace InControl
{
    ...
    public class TestInputManager : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            InputManager.OnActiveDeviceChanged +=
                inputDevice => Debug.Log("Active device changed to: " + inputDevice.Name);
        ...
        void OnGUI()
        {
            ...
            foreach (var inputDevice in InputManager.Devices)
            {
                ...
                {
                    var nativeDeviceInfo = String.Format("VID = 0x{0:x}, PID = 0x{1:x}, VER = 0x{2:x}",
                        nativeDevice.Info.vendorID, nativeDevice.Info.productID, nativeDevice.Info.versionNumber);
                ...
                if (inputDevice.IsKnown)
                {
                    ...
                    label = string.Format("{0} {1}", "Left Stick A",
                        inputDevice.LeftStick.State ? "= " + inputDevice.LeftStick.Angle : "");
                    GUI.Label(new Rect(x, y, x + w, y + 10), label, style);
                    y += lineHeight;
                    ...
                    label = string.Format("{0} {1}", "Right Stick A",
                        inputDevice.RightStick.State ? "= " + inputDevice.RightStick.Angle : "");
                    GUI.Label(new Rect(x, y, x + w, y + 10), label, style);
                    y += lineHeight;
        ...
        void DrawUnityInputDebugger()
        {
            ...
            for (var i = 0; i < numJoysticks; i++)
            {
                ...
                GUI.Label(new Rect(x, y, x + w, y + 10), "Joystick " + joystickId + ": \"" + joystickName + "\"",
                    style);
                y += lineHeight;
