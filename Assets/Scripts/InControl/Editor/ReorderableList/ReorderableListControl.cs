...
namespace InControl.ReorderableList
{
    ...
    public class ReorderableListControl
    {
        ...
        [SerializeField] private ReorderableListFlags
            _flags;
        ...
        [SerializeField] private GUIStyle
            _containerStyle;
        ...
        private void DrawListItem(EventType eventType, Rect position, IReorderableListAdaptor adaptor, int itemIndex)
        {
            ...
            if (eventType == EventType.Repaint && visible)
            {
                // Draw grab handle?
                if (draggable)
                    GUI.DrawTexture(new Rect(position.x + 6, position.y + position.height / 2f - 3, 9, 5),
                        ReorderableListResources.texGrabHandle);

                // Draw splitter between list items.
                if (!_tracking || itemIndex != s_AnchorIndex)
                    GUI.DrawTexture(new Rect(position.x, position.y - 1, position.width, 1),
                        ReorderableListResources.texItemSplitter);
            }
            ...
            try
            {
                ...
                if (eventType == EventType.ContextClick && position.Contains(Event.current.mousePosition) &&
                    (flags & ReorderableListFlags.DisableContextMenu) == 0)
                {
        ...
        private void DrawFloatingListItem(EventType eventType, IReorderableListAdaptor adaptor,
            ...
            switch (eventType)
            {
                ...
                case EventType.MouseDrag:
                    if (_tracking)
                    {
                        ...
                        s_DragItemPosition.y = Mathf.Clamp(mousePosition.y + s_AnchorMouseOffset, firstItemY,
                            position.yMax - s_DragItemPosition.height - 1);
                    }
                    break;
