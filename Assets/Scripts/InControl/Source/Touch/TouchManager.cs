namespace InControl
{
    ...
    public class TouchManager : SingletonMonoBehavior<TouchManager, InControlManager>
    {
        ...
        [Space(10)] public Camera touchCamera;
        public GizmoShowOption controlsShowGizmos = GizmoShowOption.Always;

        [HideInInspector] public bool enableControlsOnTouch = false;

        [SerializeField, HideInInspector] bool _controlsEnabled = true;

        // Defaults to UI layer.
        [HideInInspector] public int controlsLayer = 5;
...
#pragma warning restore 414
        ...
        Vector3 ConvertViewToWorldPoint(Vector2 point)
        {
            if (TouchCameraIsValid())
            {
                return touchCamera.ViewportToWorldPoint(
                    new Vector3(point.x, point.y, -touchCamera.transform.position.z));
            }
            else
            {
                return Vector3.zero;
        ...
        Vector3 ConvertScreenToViewPoint(Vector2 point)
        {
            if (TouchCameraIsValid())
            {
                return touchCamera.ScreenToViewportPoint(new Vector3(point.x, point.y,
                    -touchCamera.transform.position.z));
            }
            else
            {
                return Vector3.zero;
