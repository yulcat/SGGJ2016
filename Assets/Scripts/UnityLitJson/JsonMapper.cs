...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        private static ObjectMetadata AddObjectMetadata(Type type)
        {
            ...
            BindingFlags pflags = BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public |
                ...
                if (aliasAttrs.Length > 0)
                {
                    ...
                    if (aliasAttr.Alias == pinfo.Name)
                    {
                        throw new JsonException(string.Format(
                            "Alias name '{0}' must be different from the property it represents for type '{1}'",
                            pinfo.Name, type));
                    }
                    if (data.Properties.ContainsKey(aliasAttr.Alias))
                    {
                        throw new JsonException(string.Format("'{0}' already contains the property or alias name '{1}'",
                            type, aliasAttr.Alias));
                    }
            ...
            BindingFlags fflags = BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public |
                                  BindingFlags.NonPublic;
            foreach (FieldInfo finfo in type.GetFields(fflags))
            {
                ...
                if (aliasAttrs.Length > 0)
                {
                    ...
                    {
                        throw new JsonException(string.Format(
                            "Alias name '{0}' must be different from the field it represents for type '{1}'",
                            finfo.Name, type));
                    }
                    if (data.Properties.ContainsKey(aliasAttr.Alias))
                    {
                        throw new JsonException(string.Format("'{0}' already contains the field or alias name '{1}'",
                            type, aliasAttr.Alias));
                    }
                    pdata.Alias = aliasAttr.Alias;
                    if (aliasAttr.AcceptOriginal)
...
patch for WinRT build targets, see Platform.cs */
			if (valueType.IsEnum()) {
                ...
                // No luck
                throw new JsonException(string.Format("Can't assign value '{0}' (type {1}) to type {2}", reader.Value,
                    jsonType, instType));
            }
                ...
                else
                {
                    ...
                    if (reader.TypeHinting && property == reader.HintTypeName)
                    {
                        ...
                        if ((string) reader.Value == reader.HintValueName)
                        {
                            ...
                            {
                                throw new JsonException(string.Format(
                                    "Invalid type hinting object, has too many properties: {0}...", reader.Token));
                            }
                            return value;
                        }
                        else
                        {
                            throw new JsonException(string.Format(
                                "Expected \"{0}\" property for type hinting but instead got \"{1}\"",
                                reader.HintValueName, reader.Value));
                        }
                    }
                }
                ...
                while (!done)
                {
                    ...
                    else
                    {
                        if (!tdata.IsDictionary)
                        {
                            ...
                            {
                                throw new JsonException(string.Format("The type {0} doesn't have the property '{1}'",
                                    instType, property));
                            }
                            else
                            {
        ...
        {
            RegisterImporter(baseImportTable, typeof(long), typeof(sbyte),
                delegate(object input) { return Convert.ToSByte((long) input); });
            RegisterImporter(baseImportTable, typeof(long), typeof(byte),
                delegate(object input) { return Convert.ToByte((long) input); });
            RegisterImporter(baseImportTable, typeof(long), typeof(short),
                delegate(object input) { return Convert.ToInt16((long) input); });
            RegisterImporter(baseImportTable, typeof(long), typeof(ushort),
                delegate(object input) { return Convert.ToUInt16((long) input); });
            RegisterImporter(baseImportTable, typeof(long), typeof(int),
                delegate(object input) { return Convert.ToInt32((long) input); });
            RegisterImporter(baseImportTable, typeof(long), typeof(uint),
                delegate(object input) { return Convert.ToUInt32((long) input); });
            RegisterImporter(baseImportTable, typeof(long), typeof(ulong),
                delegate(object input) { return Convert.ToUInt64((long) input); });
            RegisterImporter(baseImportTable, typeof(long), typeof(float),
                delegate(object input) { return Convert.ToSingle((long) input); });
            RegisterImporter(baseImportTable, typeof(long), typeof(double),
                delegate(object input) { return Convert.ToDouble((long) input); });
            RegisterImporter(baseImportTable, typeof(long), typeof(decimal),
                delegate(object input) { return Convert.ToDecimal((long) input); });
            RegisterImporter(baseImportTable, typeof(double), typeof(float),
                delegate(object input) { return Convert.ToSingle((double) input); });
            RegisterImporter(baseImportTable, typeof(double), typeof(decimal),
                delegate(object input) { return Convert.ToDecimal((double) input); });
            RegisterImporter(baseImportTable, typeof(string), typeof(char),
                delegate(object input) { return Convert.ToChar((string) input); });
            RegisterImporter(baseImportTable, typeof(string), typeof(DateTime),
                delegate(object input) { return Convert.ToDateTime((string) input, datetimeFormat); });
        }

        private static void RegisterImporter(IDictionary<Type, IDictionary<Type, ImporterFunc>> table, Type jsonType,
            Type valueType, ImporterFunc importer)
        {
        ...
        private static void WriteValue(object obj, JsonWriter writer, bool privateWriter, int depth)
        {
            ...
            {
                throw new JsonException(string.Format(
                    "Max allowed object depth reached while trying to export from type {0}", obj.GetType()));
            }
            if (obj == null)
            {
                writer.Write(null);
        ...
        public static JsonData ToObject(TextReader reader)
        {
            ...
            return (JsonData) ToWrapper(delegate { return new JsonData(); }, jsonReader);
        }
        ...
        {
            ExporterFunc wrapper = delegate(object obj, JsonWriter writer) { exporter((T) obj, writer); };
            customExportTable[typeof(T)] = wrapper;
        }

        public static void RegisterImporter<TJson, TValue>(ImporterFunc<TJson, TValue> importer)
        {
            ImporterFunc wrapper = delegate(object input) { return importer((TJson) input); };
